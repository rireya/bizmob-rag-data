/**
 * bizMOB 4.0 Cross-Platform Mobile Development SDK
 *
 * @fileoverview bizMOB은 모바일 앱과 웹 환경에서 통합된 개발 경험을 제공하는 Cross-Platform Mobile Development SDK입니다.
 * 네이티브 모바일 앱과 웹 애플리케이션 간의 차이를 추상화하여 개발자가 하나의 코드베이스로
 * 두 환경 모두에서 동작하는 애플리케이션을 개발할 수 있게 합니다.
 *
 * @description 주요 특징:
 ...
 * - **Mock 시스템**: 개발 단계에서 네이티브 API 없이도 테스트 가능한 Mock 데이터 지원
 ...
 * - **bizMOB-xross4.js** (본 파일): 통합 클라이언트 라이브러리 인터페이스
 *   ├─ 환경 감지 및 Gateway 라우팅 담당
 *   ├─ bizMOB-core.js 또는 bizMOB-core-web.js로 API 호출 전달
 *   └─ 모든 bizMOB API의 진입점 역할
 *
 * - **bizMOB-core.js**: 앱 환경용 Core 모듈
 *   ├─ 네이티브 API와 JavaScript 브릿지 구현
 *   ├─ 앱 컨테이너 내에서만 동작
 ...
 * **실행 흐름**:
 * ```
 * TypeScript Wrapper → bizMOB-xross4.js → Gateway →
 * ├─ [앱 환경] bizMOB-core.js → Native API
 * └─ [웹 환경] bizMOB-core-web.js → Browser API
 * ```
 *
 ...
 *
 * @example
 * // TypeScript 래퍼 서버 통신 async/await 패턴
